...
def _total_param(model):
    param_list = list(model.parameters())
    total_params = sum(par.numel() for par in param_list)
    return total_params
...
def _natural_sample_categorical(categorical_probs):
    batch_size, sequence_length, vocab_size = categorical_probs.shape
    
    # Normalize probabilities along the last dimension
    normalized_probs = F.normalize(categorical_probs, p=1, dim=-1)  # Ensure sum along vocab_size is 1
    # Flatten for sampling
    flattened = normalized_probs.contiguous().view(-1, vocab_size)  # Shape: [(B*L), V]
    # Sample one token per row
    sampled = torch.multinomial(flattened, num_samples=1)  # Shape: [(B*L), 1]
    # Reshape back to (B, L)
    sampled = sampled.view(batch_size, sequence_length)  # Shape: [B, L]
    return sampled

def _natural_sample_categorical_distrib(categorical_probs):
    categorical_probs = torch.clamp(categorical_probs, min=1e-10)
    normalized_probs = categorical_probs / categorical_probs.sum(dim=-1, keepdim=True)
...
class Diffusion(L.LightningModule):
  ...
  def _subs_parameterization_no_mask(self, logits, xt):
    logits_normalized = logits - torch.logsumexp(logits, dim=-1, keepdim=True)
  ...
  def forward(self, x, sigma, full_scores = False):
    """Returns log score."""
    sigma = self._process_sigma(sigma)

    with torch.cuda.amp.autocast(dtype=torch.float32):
      logits = self.backbone(x, sigma)
    if self.parameterization == 'subs' and full_scores == False:
      return self._subs_parameterization(logits=logits, xt=x)
    elif self.parameterization == 'subs' and full_scores == True:
      return self._subs_parameterization_no_mask(logits=logits, xt=x)
    return logits
  ...
  def _initialize_batch_of_samples(self,batch_size):
    return self._sample_prior(batch_size,self.config.model.length).to(self.device)
  ...
  def _ddpm_update_finetune_gradient(self, x, t, dt, copy_flag_temp, return_process=False):
    ...
    q_xs = q_xs.clone()
    q_xs[:, :, self.mask_index] = move_chance_s[:, :, 0]
    _x = _natural_sample_categorical_distrib(q_xs.clone())
    ...
    if return_process:
      return (soft_copy_flag * x + (1 - soft_copy_flag) * _x).clone(), x, unet_conditioning, move_chance_t, soft_copy_flag
    else:
      _x = _x.to(x.device)
      soft_copy_flag = soft_copy_flag.to(x.device)
      return (soft_copy_flag * x + (1 - soft_copy_flag) * _x).clone()
  ...
  def _ddpm_update(self, x, t, dt, return_process=False):
    ...
    q_xs = log_p_x0.exp() * (move_chance_t
                             - move_chance_s)
    q_xs[:, :, self.mask_index] = move_chance_s[:, :, 0]
    _x  = _natural_sample_categorical(q_xs)
    copy_flag = (x != self.mask_index).to(x.dtype)
  ...
  def _build_distrib(self, x, t, dt, full_scores=False):
    sigma_t, _ = self.noise(t)
    sigma_s, _ = self.noise(t - dt)
    if sigma_t.ndim > 1:
      sigma_t = sigma_t.squeeze(-1)
    ...
    move_chance_s = move_chance_s[:, None, None]
    unet_conditioning = sigma_t
    log_p_x0 = self.forward(x, unet_conditioning, full_scores=full_scores)
    assert move_chance_t.ndim == log_p_x0.ndim
    q_xs = log_p_x0.exp() * (move_chance_t
                             - move_chance_s)
    q_xs[:, :, self.mask_index] = move_chance_s[:, :, 0]
  ...
  def _sample_no_gradient(self, num_steps=None, eps=1e-5, eval_sp_size=None, copy_flag_temp=None):
    ...
    for i in range(num_steps):
        ...
        if i < num_steps - self.config.finetuning.truncate_steps:
            ...
            x, last_x, condt, move_chance_t, copy_flag = self._ddpm_update_finetune_gradient(
  ...
  def _compute_loss_sepo(self, rew, s_t_old_x_z, pi_old_z, x, t=0.0, epsilon=0.2, full_scores=True):
    ...
    log_s_t_x_y = self.forward_at_time(x, t, full_scores=full_scores)
  ...
  def _compute_entropy_bonus(self, x, t, full_scores=True, eps_error=1e-8):
    x = x.detach()

    # Compute the probability distribution pi_y
    pi_y = self._build_distrib(x, t * torch.ones(x.shape[0], 1, device=self.device), t, full_scores=full_scores)

    # Remove the last category (we don't take tha mask into account)
    pi_y_attach_truncated = pi_y[:, :, :-1]  # Shape: [B, L, d-1]

    # Normalize along the last dimension (avoid division by zero)
    norm = pi_y_attach_truncated.sum(dim=-1, keepdim=True)
    pi_y_attach_truncated = torch.where(norm > 0, pi_y_attach_truncated / norm, torch.zeros_like(pi_y_attach_truncated))

    # Compute entropy: -Σ p log p
    log_probs = torch.log(torch.clamp(pi_y_attach_truncated, min=eps_error))
    entropy_ = -(pi_y_attach_truncated * log_probs).sum(dim=-1).mean()  # Mean over B and L

    return entropy_

  def forward_at_time(self, x, t: float, full_scores=True) -> torch.Tensor:

    batch_size = x.shape[0]
    t_tensor = x.new_full((batch_size, 1), t)

    sigma_t, _ = self.noise(t_tensor)

    output = self.forward(x, sigma_t, full_scores=full_scores)
  ...
  def get_score_time(self, x,t):
    # 1) On construit un tenseur t_tensor de shape [batch] ou [batch, 1]
    batch_size = x.shape[0]
    t_tensor = x.new_full((batch_size, 1), t)  # un broadcast du scalaire t

    # 2) On obtient sigma_t et potentiellement dsigma
    sigma_t, _ = self.noise(t_tensor)  # shape [batch_size, 1]
    return self.get_score(x,sigma_t)
